...
class Funcionario
{
    ...
    public function save()
    {
        ...
        if (!isset($this->id)) {
            ...
            $query = "UPDATE funcionarios SET ".implode(', ', $definir)." WHERE id='{$this->id}';";
        }
    ...
    {
        $conexao = Conexao::getInstance();
        $count   = $conexao->exec("SELECT count(*) FROM funcionarios;");
        if ($count) {
            return (int) $count;
        }
        return false;
    ...
    {
        $conexao = Conexao::getInstance();
        if ($conexao->exec("DELETE FROM funcionarios WHERE id='{$id}';")) {
            return true;
        }
        return false;
    }
